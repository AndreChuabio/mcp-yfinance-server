...
async def place_buy_order(symbol: str, shares: float) -> list[TextContent]:
    ...
    try:
        ...
        if total_cost > PAPER_PORTFOLIO["cash_balance"]:
            raise ValueError(
                f"Insufficient funds. Need ${total_cost:.2f}, have ${PAPER_PORTFOLIO['cash_balance']:.2f}")

        # Execute the trade
        PAPER_PORTFOLIO["cash_balance"] -= total_cost

        if symbol in PAPER_PORTFOLIO["positions"]:
            # Add to existing position
            existing = PAPER_PORTFOLIO["positions"][symbol]
            total_shares = existing["shares"] + shares
            total_cost_basis = (existing["shares"]
                                * existing["avg_cost"]) + total_cost
        ...
        PAPER_PORTFOLIO["orders"].append(order)
...
**Updated Portfolio:**
**Cash Balance:** ${PAPER_PORTFOLIO['cash_balance']:,.2f}
**{symbol} Position:** {PAPER_PORTFOLIO['positions'][symbol]['shares']} shares @ ${PAPER_PORTFOLIO['positions'][symbol]['avg_cost']:.2f} avg cost
"""
...
async def place_sell_order(symbol: str, shares: float) -> list[TextContent]:
    ...
    try:
        ...
        if shares > position["shares"]:
            raise ValueError(
                f"Cannot sell {shares} shares, only have {position['shares']}")
        ...
        # Execute the trade
        PAPER_PORTFOLIO["cash_balance"] += total_proceeds
        ...
        if remaining_shares == 0:
            ...
            PAPER_PORTFOLIO["positions"][symbol]["shares"] = remaining_shares
...
"""
        ...
        if symbol in PAPER_PORTFOLIO["positions"]:
            result += f"**{symbol} Position:** {PAPER_PORTFOLIO['positions'][symbol]['shares']} shares remaining"
        else:
            result += f"**{symbol} Position:** Closed"
