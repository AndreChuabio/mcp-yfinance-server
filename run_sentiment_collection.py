...
def run_collection_for_stock(symbol: str, company_name: str, 
    ...
    try:
        ...
        if verification.get('current_sentiment'):
            ...
            logger.info(f"  Current sentiment: {sent.get('label')} ({sent.get('score'):.2f})")
    ...
    try:
        ...
        if args.symbol:
            ...
            if symbol not in PORTFOLIO_STOCKS:
                ...
                logger.info(f"Valid symbols: {', '.join(PORTFOLIO_STOCKS.keys())}")
                sys.exit(1)
        ...
        logger.info("\n" + "="*60)
        logger.info("Collection complete. View data at https://console.neo4j.io")
        logger.info("="*60 + "\n")
