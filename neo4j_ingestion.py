...
class Neo4jIngestion:
    ...
    def validate_article_data(self, article_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not stock_symbol:
                logger.warning(f"Article missing stock symbol: {article.get('url')}")
                return False
            
            if not isinstance(sentiment.get('score'), (int, float)):
                logger.warning(f"Invalid sentiment score: {sentiment.get('score')}")
                return False
            
            if sentiment['score'] < -1.0 or sentiment['score'] > 1.0:
                logger.warning(f"Sentiment score out of range: {sentiment['score']}")
                return False
            
            if sentiment.get('label') not in ['bullish', 'bearish', 'neutral']:
                logger.warning(f"Invalid sentiment label: {sentiment.get('label')}")
                return False
    ...
    def ingest_batch(self, articles_data: List[Dict[str, Any]], batch_size: int = 15) -> Dict[str, int]:
        ...
        logger.info(f"Validated {len(valid_articles)}/{len(articles_data)} articles")
    ...
    def verify_ingestion(self, stock_symbol: str) -> Dict[str, Any]:
        ...
        try:
            current_sentiment = self.connector.get_stock_sentiment(stock_symbol)
            results['current_sentiment'] = current_sentiment
            
            recent_articles = self.connector.get_recent_articles_for_stock(stock_symbol, limit=5)
...
def main():
    ...
    try:
        ...
        stats = ingestion.ingest_for_stock('NVDA', 'NVIDIA Corporation', articles)
